...
The fastest and most affordable model, with open weights that run on consumer GPUs. 9B trades some speed for higher quality. See [which model to use](/flux_2/flux2_overview#which-model-to-choose).
...
paths:
  /v1/flux-2-klein-4b:
    post:
      ...
      description: >-
        Submits an image generation or editing task with FLUX.2 [klein] 4B. The
        fastest, most lightweight FLUX.2 model, with open weights that run on
        consumer GPUs.
